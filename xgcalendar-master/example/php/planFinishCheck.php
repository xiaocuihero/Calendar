...
$resultPlan = $db->query("SELECT 1 FROM `calendar` c WHERE c.StartTime='{$checkStartDate}' AND c.EndTime='{$checkStartDate}' AND UpAccount='{$userid}' AND c.`Category`=0");
...
$resultFinish = $db->query("SELECT 1 FROM `calendar` c WHERE c.StartTime='{$checkStartDate}' AND c.EndTime='{$checkStartDate}' AND UpAccount='{$userid}' AND c.`Category`=1");
